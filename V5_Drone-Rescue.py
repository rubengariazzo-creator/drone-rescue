...
def deplacer_drone(d, dest, score, journal, tour):
    ...
    if d["batt"] < 0:
        ...
        if d["etat"] == "actif":
            d["etat"] = "immobile"
            journal.append(f"Tour {tour} : D{d['id']} à court de batterie et s'est immobilisé")
    # Récupération du survivant
    for i, s in enumerate(survivants):
        if s["pos"] == (r, c) and d["charge"] is None:
            d["charge"] = s["id"]
            del survivants[i]
            journal.append(f"Tour {tour} : D{d['id']} récupère S{s['id']}")
            break
    # Dépser à l'hopi
        ...
        if 0 <= nr < T and 0 <= nc < T and (nr, nc) not in batiments:
            ...
            for d in drones:
                if d["pos"] == (nr, nc) and d["etat"] == "actif":
                    d["etat"] = "desactive"
                    d["desact"] = 2
                    journal.append(f"Tour {tour} : Tempête {t['id']} touche D{d['id']}, il a été désactivé 2 tours")
            return journal
    return journal
